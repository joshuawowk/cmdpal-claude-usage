...
|  |  |
|:--:|:--:|
| ![Dock tile](docs/store-listing/screenshots/01-dock-tile.png) | ![Usage tab](docs/store-listing/screenshots/02-usage.png) |
| **Dock tile** — live session &amp; weekly usage remaining | **Usage** — session, weekly &amp; per-model bars |
| ![Breakdown tab](docs/store-listing/screenshots/03-breakdown.png) | ![Heatmap tab](docs/store-listing/screenshots/04-heatmap.png) |
...
docs/
  ...
  store-listing/                authored Store-listing source (tracked)
    store-listings.csv          per-language listing copy (description, features, …)
    store-screenshot-captions.csv  per-language screenshot captions
    screenshots/                the four 1366×768 PNGs + their HTML mockup sources (src/)
    README.md                   how the listing content and generated bundles fit together
  (store-submission/, store-import/ are generated by the scripts below and gitignored)
scripts/
  ...
  build-store-submission.ps1      StoreBroker PDP bundle from docs/store-listing/ (-> docs/store-submission/)
  fill-store-listing-csv.ps1      fills a Partner Center listing export (-> docs/store-import/)
